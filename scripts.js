...
Game.creeps[Object.keys(Game.creeps).find((creep) => Game.creeps[creep].memory.role == require('role').HARVESTER)].memory.priority = STRUCTURE_EXTENSION;
...
Object.keys(Game.creeps).filter((creep) => Game.creeps[creep].memory.role == require('role').REPAIR).length;
Object.keys(Game.creeps).filter((creep) => Game.creeps[creep].memory.priority != null).length;
